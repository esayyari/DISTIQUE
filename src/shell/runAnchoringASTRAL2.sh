...
$WS_LOC_UTIL/distique.py -g $gt1000half -o $res1000half  > $res1000half/results.log 2>&1
echo "working on $i/estimatedgenetre.halfresolved1000 has been finished!"
$WS_LOC_UTIL/distique.py -g $gt200half  -o $res200half   > $res200half/results.log 2>&1
echo "working on $i/estimatedgenetre.halfresolved200 has been finished!"
$WS_LOC_UTIL/distique.py -g $gt50half   -o $res50half    > $res50half/results.log 2>&1
echo "working on $i/estimatedgenetre.halfresolved50 has been finished!"
$WS_LOC_UTIL/distique.py -g $gt1000true -o $res1000true  > $res1000true/results.log 2>&1
echo "working on $i/truegenetre1000 has been finished!"
$WS_LOC_UTIL/distique.py -g $gt200true  -o $res200true > $res200true/results.log 2>&1
echo "working on $i/truegenetre200 has been finished!"
$WS_LOC_UTIL/distique.py -g $gt50true   -o $res50true  > $res50true/results.log 2>&1
echo "working on $i/truegenetre50 has been finished!"
for x in `find $tmpDIR -name "distance.d_distique_tree*"`; do
	y=$(dirname $x)
	$WS_HOME/DISTIQUE/src/shell/compare.tree.sh -s $sp -g $x > $y/results-distique.score
done
y=$(echo $o | sed -e 's/^.*\///')
tar czf $o/testAnchoring-all.tar.gz $tmpDIR/* 
rm -r $tmpDIR
