...
for file in `find $path_tmp -type f -name "*.gt"`; do
	...
	f_tmp=$( basename "${file}" )
	if [ -d $out/$tmp_out/distique-2/$method ]; then
		printf "the folder $out/$tmp_out/distique-2/$method already exists\n"
	else
		printf "the folder $out/$tmp_out/distique-2/$method created\n"
		mkdir -p $out/$tmp_out/distique-2/$method
	fi
