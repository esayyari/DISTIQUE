...
#	fi
	if [ ! -s $out/$tmp_out/distique-cons/$method/distance.d_distique_tree.nwk ]; then
		printf "mkdir -p ./$out/$tmp_out/distique-cons/$method/; source /etc/profile.d/modules.sh; module load python; module load scipy; VIRTUAL_ENV_DISABLE_PROMPT=1; source $WS_HOME/python27-gordon/bin/activate; /usr/bin/time -po ./$out/$tmp_out/distique-cons/$mehtod/$f_tmp-log.info $DIR_UTILS/distique.py -g $file -m $method  -o ./$out/$tmp_out/distique-cons/$method -x 2; mv ./$out/$tmp_out/distique-cons/$method $out/$tmp_out/distique-cons/$method\n">>tasks.massive-$method.3
	fi
