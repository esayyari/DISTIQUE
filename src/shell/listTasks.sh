...
for file in `find $path_tmp -type f -name "*.gt"`; do
	...
	if [ ! -s $out/$tmp_out/distance.d_fastme_tree.nwk ]; then
	printf "source /etc/profile.d/modules.sh; module load python; module load scipy; VIRTUAL_ENV_DISABLE_PROMPT=1; source $WS_HOME/python27-gordon/bin/activate;/usr/bin/time -po $out/$tmp_out/$f_tmp-log.info $DIR_DISTIQUE/summarize.tree.sh -s $summethod -d $method -w 1 -r $out/$tmp_out $file\n">>tasks.massive2
	fi
