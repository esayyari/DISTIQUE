...
/usr/bin/time -p $WS_LOC_UTIL/distique-v$v"."py -g $gt1000half -o $res1000half -n $n -u fastme -z $z > $res1000half/results.log 2>&1
echo "working on $i/estimatedgenetre.halfresolved1000 has been finished!"
fi
if [ -s $gt200half ]; then
/usr/bin/time -p $WS_LOC_UTIL/distique-v$v"."py -g $gt200half  -o $res200half  -n $n -u fastme -z $z > $res200half/results.log 2>&1
...
y=$(echo $o | sed -e 's/^.*\///')
tar czf $o/distique-run2-V$v-$y-$n-$z-fastme.tar.gz $tmpDIR/* 
rm -r $tmpDIR
