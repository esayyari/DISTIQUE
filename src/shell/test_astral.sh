...
out=/oasis/projects/nsf/uot136/esayyari/old-outputs/
...
for file in `find $path_tmp -type f -name "*.gt"`; do
	...
	printf " mkdir -p ./$out/$tmp_out/astral;  /usr/bin/time -po ./$out/$tmp_out/astral/$f_tmp-log.info java -Xmx2000M -jar $DIR_Astral/astral.4.8.3.jar -t 2 -i $file -q $out/$tmp_out/astral/distance.d_astral_tree.nwk> ./$out/$tmp_out/astral/astral_tree.nwk ;cp -r ./$out/$tmp_out/astral/* $out/$tmp_out/astral/ \n">>tasks.masive
	fi
done
