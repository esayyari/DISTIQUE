...
for x in `find $wrkroot/$dataset -maxdepth 1 -type d -name "*X*"`; do
	for i in {1..20}; do
		...
		if [ -s $wrkroot/$x/R$i/distique-1/min/min/distance.d_fastme_tree.txt ]; then
			printf "mkdir -p $resroot/$x; module load python; module load scipy; $DIR/compare.tree.sh  -s $sp -g $wrkroot/$x/R$i/distique-1/min/min/distance.d_fastme_tree.txt > $resroot/$x/$dataset-min-fastme-$tmp_x-R$i.txt\n" >>list.comp.txt;
			printf "mkdir -p $resroot/$x; working on $resroot/$x/$dataset-min-fastme-$tmp_x-$method-R$i.txt has been finished.\n"
		else
                        printf "mkdir -p $resroot/$x; $wrkroot/$x/R$i/distique-1/min/min/distance.d_fastme_tree.nwk was not found \n"
